...
Marathon will continue to consume the same app JSON as in Marathon 1.4;
app JSON that uses deprecated fields continues to be normalized, internally, to the canonical representation.
However, the app JSON generated by the /v2 REST API has changed: only canonical fields are generated.
...
Operators are encouraged to back up their existing applications **before an upgrade** since it is not trivial to roll back migrated apps to the 1.4 API.
